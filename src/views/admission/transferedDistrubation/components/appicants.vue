<template>
  <div>
    <statistics :loading="load" v-if="details" :items="details" :widths="[3, 3, 3, 3, 3, 3, 3, 3, 3, 3,6]" :only="['INCOMPLETE', 'SUBMITTED', 'FEEDBACK', 'UPDATED', 'REVIEWED', 'DISTRIBUATION_REQUIRED','EQUIVALENCE_DONE', 'REJECTED', 'FINAL_FEES_REQUIRED', 'APPROVED', 'total']" :icon="'TrendingUpIcon'"></statistics>
    <DataTable :total="total"  default-filter="transfer_term_id" :paramsLoaded="paramsLoaded" :allow-search="true" :filter="filter" @reset="filter = { export: 0 }" @Refresh="refresh" @Export="Export">
      <template v-if="lookups" #filter>
        <b-col cols="12" md="4">
          <v-select v-model="filter.transfer_term_id" :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" :options="lookups.terms" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Global.term')">
        ...
        <b-col cols="12" md="4">
          <v-select :filter="fuseSearch" v-model="filter.country_id" :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" :options="appLookups.countries" class="w-100 mb-2" label="name" :reduce="val => val.id" :placeholder="$t('Global.country')">
            <template v-slot:option="option">
              {{ $i18n.locale == 'ar' ? option.name_local : option.name }}
            </template>
            <template slot="selected-option" slot-scope="option">
        ...
        <b-col cols="12" md="4">
          <v-select :filter="fuseSearch" v-model="filter.faculty_reference_id" :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" :options="lookups.faculties" label="name" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Global.faculty_preference')">
            <template slot="selected-option" slot-scope="option">
              <span v-if="$i18n.locale == 'ar'">
                {{ option.name_local || option.name }}
              </span>
         ...
         <b-col cols="12" md="4">
          <v-select v-model="filter.egyptian" :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" :options="lookups.egyptians_nonegyptians" label="name" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Egyptians / Non-Egyptians')">
            <template slot="selected-option" slot-scope="option">
              <span v-if="$i18n.locale == 'ar'">
                {{ option.name_local || option.name }}
              </span>
      ...
      <template #customTable>
        <b-table :busy="load" @sort-changed="sortingChanged"
          ...
          <template #cell(name)="data">
            <b-media vertical-align="center" class="d-inline-flex align-items-center">
              <template v-if="data.item.name" #aside>
                <b-form-checkbox v-model="selected" style="margin-top: 6px" :value="data.item.id" class="d-flex align-items-center custom-control-primary" />
              ...
              <b-link v-if="$i18n.locale == 'ar'" class="font-weight-bold d-inline-block" :to="{ name: 'applicant', params: { id: data.item.id } }">
                {{ data.item && data.item.name_local ? data.item.name_local : '-' }}
              </b-link>
              <b-link v-if="$i18n.locale == 'en'" class="font-weight-bold d-inline-block" :to="{ name: 'applicant', params: { id: data.item.id } }">
            ...
            <div v-if="data.item.desires && data.item.desires.desire_one && data.item.desires.desire_one.faculty">
              ...
              <b-badge active pill variant="primary">
                <span v-if="data.item.desires.desire_one.program_setting.quota">
                  {{ $t('Global.program') }}
                  ( {{ data.item.desires.desire_one.program_setting.count }} /{{ data.item.desires.desire_one.program_setting.quota }} )
                </span>
                <span v-else> ( {{ data.item.desires.desire_one.setting.count }} /{{ data.item.desires.desire_one.setting.quota }} ) </span>
              </b-badge>
            </div>
          </template>
          ...
          <template #cell(previous)="data">
                ...
                <span v-if="data.item.previous_university_info"> {{ data.item.previous_university_info.name_previous_university }} </span><br>
                
                <span v-if="data.item.previous_university_info.bylaw_type == false">
                  {{$t('Semester Based')}}
                </span>
                <span v-else-if="data.item.previous_university_info.bylaw_type == true">
                  {{$t('Credit Hours')}}
                </span>
          ...
          <template #cell(status)="data">
            <b-badge v-if="$i18n.locale == 'ar'" pill :variant="getStatus(data.item.status ? data.item.status.id : 0).color" class="text-capitalize">
              {{ data.item.status.name_local }}
            </b-badge>
            <b-badge v-if="$i18n.locale == 'en'" pill :variant="getStatus(data.item.status ? data.item.status.id : 0).color" class="text-capitalize">
      ...
      <template #action>
        <b-button @click="filter.export = 1" v-if="hasPermission('export_applicant')" v-b-tooltip.hover="$t('Global.export_excel')" variant="primary" class="btn-icon">
          <feather-icon icon="FileTextIcon" />
        </b-button>

        <b-button @click="bulk" v-if="hasPermission('distribute_applicant')" :disabled="selected.length < 1" v-b-tooltip.hover="$t('Global.distribution')" variant="primary" class="btn-icon ml-1">
...
<script>
import { BCard, BRow, BCol, BFormInput, BFormGroup, BButton, BTable, BFormCheckbox, BFormCheckboxGroup, BMedia, BAvatar, BLink, BBadge, BDropdown, BSpinner, BDropdownItem, BPagination, BOverlay, BCardHeader, BCardBody, BModal, VBModal, BPopover } from 'bootstrap-vue';
...
export default {
  ...
  data() {
    return {
      link: '',
      filter: { removed: 0, export: 0,transfer_term_id:null,pre_university_certificate_groupe:null,faculty_reference_id:null,year:null,country_id:null,pre_university_certificate_type:null,pre_university_certificate:null},
      selected: [],
  ...
  methods: {
    ...
    Export(query) {
      query = { ...query, status: 3 ,language: this.$i18n.locale };
...
[dir='rtl'] .table.b-table > thead > tr > [aria-sort]:not(.b-table-sort-icon-left),
[dir='ltr'] .table.b-table > tfoot > tr > [aria-sort]:not(.b-table-sort-icon-left) {
