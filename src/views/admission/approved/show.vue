<template>
  <b-row>
    ...
    <b-col cols="12" xl="12" lg="8" md="7">
      <b-row v-if="mainData">
        <b-col cols="12">
          <list v-if="applicant" class="pb-0" :title="$t('Global.student')" :data="applicant" :only="[]" nopading :loading="load">
            <template #edit>
              <div>
                <b-button v-if="hasPermission('assign_applicant')" @click="openAssignModel" v-b-tooltip.hover="$t('Global.assign_to_program')" variant="primary" size="sm" class="mx-sm-1">
                  {{ $t('Global.assign_to_program') }}
                </b-button>
              </div>
            </template>
          </list>
    ...
    <b-col v-if="applicant" cols="12">
      <b-card class="mb-0">
        <b-tabs v-model="tabIndex" @input="saveStatus" lazy vertical nav-wrapper-class="nav-hidden" align="left">
          ...
          <!--                        <b-card v-if="applicant.faculty && lookups" v-for="(program,i) in lookups.programs"-->
          <!--                                class="border">-->
          <!--                            <b-row class="d-flex justify-content-between px-2" :key="i">-->
          <!--                                <div class="d-flex flex-column">-->
          <!--                                    <strong>{{$i18n.locale == 'ar' ? program.name_local : program.name}}</strong>-->
          <!--                                </div>-->
          <!--                                <div>-->
          <!--                                    <div>-->
          <!--                                        <b-button type="submit" v-if="!applicant.program"-->
          <!--                                                  @click="assignProgram(program)" variant="success" class="mr-1">-->
          <!--                                            {{ $t('Global.assign_to_program') }}-->
          <!--                                        </b-button>-->
          <!--                                        <b-badge active pill-->
          <!--                                                 v-if="applicant.program && applicant.program.id == program.id"-->
          <!--                                                 :variant="`success`">-->
          <!--                                            {{$t('Global.assigned')}}-->
          <!--                                        </b-badge>-->
          <!--                                    </div>-->
          ...
          <b-tab>
            ...
            <!--                                    <b-card class="border">-->
            <!--                                        <a class="mb-2" fluid rounded target="_blank" :href="document.url"-->
            <!--                                           :alt="document.url.slice(5)">-->
            <!--                                            {{ document.short_name }}-->
            <!--                                        </a>-->
            <!--                                    </b-card>-->
            <!--                                </div>-->
            ...
            <!--                                    <b-card class="border">-->
            <!--                                        <b-card-body-->
            <!--                                                class="cursor-pointer align-items-center flex-column d-flex justify-content-center align-content-center"-->
            <!--                                                @click="uploadFile()">-->
            <!--                      <span>-->
            <!--                        {{ document.short_name }}-->
            <!--                      </span>-->
            <!--                                            <feather-icon v-if="!$store.state.app.load" size="4.5x"-->
            <!--                                                          icon="PlusCircleIcon"/>-->
    ...
    <b-modal ref="my-modal" v-model="assignModal" hide-footer :title="$t('Global.assign_to_program')">
      <div class="demo-vertical-spacing">
        <validation-observer ref="ruleForm">
          <b-form>
            <b-col cols="12" md="12">
              <b-form-group :label="$t('Global.program')" label-for="Message" v-if="lookups">
                <validation-provider #default="{ errors }" name="Message" :rules="'required'">
      ...
      <div class="mt-2">
        <b-col cols="12" md="12">
          <div class="d-flex justify-content-end">
            <b-button type="submit" variant="primary" class="mr-1" @click="assignProgram">
              {{ $t('Global.ok') }}
            </b-button>
...
export default {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      assign: 'applicants/assignProgram',
      getLookups: 'app/GET_LOOKUPS',
    }),
    init() {
      this.$store.dispatch('applicants/get', this.id).then(_ => {
        if (this.applicant && this.applicant.faculty) {
          this.getLookups({ faculty_id_programs: this.applicant.faculty.id });
        }
      });
    },
    changeStatus(action) {
    ...
    assignProgram() {
      let payload = {};
      payload.id = this.id;
      payload.query = this.assignForm;
      this.assign(payload).then(_ => {
        this.init();
