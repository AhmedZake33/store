<template>
  ...
  <DataTable importBtn :title="title" :total="total" :allowSearch="true" :filter="filter" :loading="load" @Refresh="refresh" @reset="filter = { export: 0 }">
    <template #filter v-if="lookups">
      <b-col cols="12" md="6">
        <v-select v-model="filter.type_id" :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" :options="lookups.types" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Global.type')">
      ...
      <b-col cols="12" md="6">
        <v-select :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" v-model="filter.removed" :options="getStatusList(true)" class="w-100" :reduce="val => val.value" :placeholder="$t('Global.status')">
          <template slot="selected-option" slot-scope="option">
            <span>
    ...
    <template #customTable>
      <b-table           @sort-changed="sortingChanged"
        ...
        <template #cell(name)="data">
          <b-link v-if="$i18n.locale == 'en'"  :to="{ name: 'term', params: { id: data.item.id } }" class="font-weight-bold d-block text-nowrap">
            {{ data.item ? data.item.name : '_' }}
          </b-link>
          <b-link :to="{ name: 'term', params: { id: data.item.id } }" v-if="$i18n.locale == 'ar'" >
        ...
        <template #cell(type)="data">
          <b-badge pill :variant="`primary`" class="text-capitalize">
            {{ data.item.type && $i18n.locale == 'en' ? data.item.type.name : data.item.type.name_local }}
          </b-badge>
        </template>
        ...
        <template #cell(actions)="data">
          <div>
            ...
            <feather-icon class="text-primary" v-b-tooltip.hover="$t('Global.edit')" icon="EditIcon" style="cursor: pointer" @click="open(data.item.id)" />
    ...
    <template #action>
      <b-button :to="{ name: 'edit_term' }" class="btn-icon" v-b-tooltip.hover="$t('Global.add')" variant="primary">
        <feather-icon icon="PlusIcon" />
      </b-button>
    </template>
  </DataTable>

  <Modal :hint="$t('Global.importMigrationData')" :link="'/terms/import'" :title="$t('Global.importMigrationData')" :show="importMigrationModal"></Modal>
</div>
</template>
