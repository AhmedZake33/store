      ...
      <validation-observer ref="simpleRules">
        <b-form ref="form" @submit.stop.prevent="save">
          <ShowComponent 
            ...
            <template #edit>
              <div class="mt-0 mb-1">
                <feather-icon v-if="hasPermission('edit_students_grades_recheck')" @click="cancel()" style="cursor: pointer"
...
export default {
  ...
  computed: {
    ...
    title() {
      return this.id
        ? this.$t("Global.grades_recheck_edit")
        : this.$t("Global.grades_recheck_add");
    },
  },
  props: {
    ...
    save() {
      this.$refs.simpleRules.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch("gradesRecheck/put", payload)
            .then((response) => {
              this.$swal({
                icon: "success",
              ...
              if(this.$route.name == "add_grades_recheck"){
                this.$router.go(-1)
              }else{
                this.init();
                this.$emit('hide');
