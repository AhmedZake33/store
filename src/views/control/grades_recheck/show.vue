                ...
                <div v-if="item && !edit">
                    <ShowComponent 
                        ...
                        <template #edit v-if="item.status.id == 0 && item.can_edit == true">
                            <div class="mt-0 mb-1">
                                <feather-icon v-if="hasPermission('edit_students_grades_recheck')" @click="openEdit()" style="cursor: pointer"
...
import Edit from "@/views/control/grades_recheck/edit.vue";
...
export default {
    ...
    methods:{
        init(id) {
            if(this.id || id){
                this.$store.dispatch("gradesRecheck/get", this.id);
