<template>
  <DataTable :total="total" :allowSearch="true" :filter="filter" @Refresh="refresh">
    <template #filter v-if="lookups">
      <b-col cols="12" md="4">
        <v-select :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" v-model="filter.bylaw_id" :options="lookups.bylaws" label="name" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Global.bylaws')" />
      </b-col>
      <b-col cols="12" md="4">
        <v-select :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" v-model="filter.program_id" :options="lookups.programs" label="name" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Global.programs')" />
      </b-col>
      <b-col cols="12" md="4">
        <v-select :dir="$store.state.appConfig.layout.isRTL ? 'rtl' : 'ltr'" v-model="filter.level_id" :options="lookups.levels" label="name" class="w-100 mb-2" :reduce="val => val.id" :placeholder="$t('Global.level')" />
      </b-col>
      <b-col cols="12" md="4">
...
export default {
  ...
  computed: {
    ...mapGetters({
      items: 'student_questionnaires/items',
      total: 'student_questionnaires/total',
      load: 'student_questionnaires/load',
      needDrop:'app/needDrop',
      lookups: 'student_questionnaires/lookups',
    }),
  },
  methods: {
    refresh(query) {
      query = { ...query, type: this.type };
      this.$store.dispatch('student_questionnaires/questionnaires', { query: query });
    },
  },
};
</script>
