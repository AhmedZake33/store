<template>
  <DataTable :title="title" :total="total" :allowSearch="true" :filter="filter" @Refresh="refresh">
    ...
    <template #customTable>
      <b-table :busy="load"           @sort-changed="sortingChanged"
          no-local-sorting class="position-relative" :items="items" responsive :fields="fieldsData" primary-key="id" show-empty :empty-text="$t('Global.empty_text')">
        <template #table-busy>
        ...
        <template #cell(question)="data">
          <b-media vertical-align="center" class="d-inline-flex align-items-center">
            <span v-if="$i18n.locale == 'ar'" v-b-tooltip.hover="data.item && data.item.name ? data.item.question_local : '-'" class="font-weight-bold d-inline-block text-nowrap" :to="{ name: 'form', params: { id: data.item.id } }">
              {{ data.item && data.item.question_local ? data.item.question_local : '-' }}
            </span>
            <span class="font-weight-bold d-inline-block text-nowrap" :to="{ name: 'form', params: { id: data.item.id } }" v-if="$i18n.locale == 'en'" v-b-tooltip.hover="data.item && data.item.question ? data.item.question : '-'">
              {{ data.item && data.item.question ? data.item.question : '-' }}
            </span>
          </b-media>
        </template>
        ...
        <template #cell(section)="data">
          <b-media vertical-align="center" class="d-inline-flex align-items-center">
            <span v-if="$i18n.locale == 'ar'" v-b-tooltip.hover="data.item && data.item.name ? data.item.section_local : '-'" class="font-weight-bold d-inline-block text-nowrap">
              {{ data.item && data.item.section_local ? data.item.section_local : '-' }}
            </span>
            <span class="font-weight-bold d-inline-block text-nowrap" v-if="$i18n.locale == 'en'" v-b-tooltip.hover="data.item && data.item.section ? data.item.section : '-'">
              {{ data.item && data.item.section ? data.item.section : '-' }}
            </span>
          </b-media>
        </template>
...
export default {
  ...
  methods: {
    refresh(query) {
      ...
      this.$store.dispatch('questions/questions', { query: query });
    },
  },
};
</script>
