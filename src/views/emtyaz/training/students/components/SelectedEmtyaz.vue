<template>
              ...
              <template #header>
                <div>
                  <h2 class="mb-0 d-inline">
                    {{ $t('Global.emtyaz')+'-'+ ($i18n.locale == 'en' ? student.student_term.name:student.student_term.name_local) }}
                  </h2>
              ...
              <b-row>
                ...
                >
                  ...
                  <template #cell(count)="data">
                    {{ data.item.currentCount }}/{{ data.item.capacity }}
                  </template>
                  <template #cell(status)="data">
                    <span v-for="status in data.item.allstatus" :key="status">
                          <b-badge
                  ...
                  <template #cell(actions)="data">
                    ...
                    >
                      <b-button
                          v-if="[1,2,4].includes(data.item.status) && hasPermission('removeRegistered_registration')"
                      ...
                      <!--         for advisor actions             -->
                      <b-button
                          v-if="[1].includes(data.item.status) && hasPermission('accept_registration')"
                          v-ripple.400="'rgba(234, 84, 85, 0.15)'"
                          variant="outline-success"
                      ...
                      <b-button
                          v-if="[1,2].includes(data.item.status) && hasPermission('reject_registration')"
                          v-ripple.400="'rgba(234, 84, 85, 0.15)'"
                          variant="outline-warning"
              ...
              >
                ...
                >
                  <div>
                    ...
                    <b-badge pill>
                      {{option.section_count}}/{{option.quota}}
                    </b-badge>
  ...
  export default {
    ...
    methods: {
      ...mapActions({
        getSelectedCoursesDetails: 'Emtyaz/getSelectedEmtyazDetails',
        removeRequestedOffering: 'Emtyaz/removeRequestedEmtyaz',
        acceptRequestedOffering: 'Emtyaz/acceptRequestedEmtyaz',
        acceptAllRequestedOfferings: 'Emtyaz/acceptAllRequestedEmtyaz',
        rejectRequestedOffering: 'Emtyaz/rejectRequestedEmtyaz',
      ...
      removeRequestedOfferingBTN(offeringId, event) {
        ...
        this.removeRequestedOffering({ emtyaz_training_id: offeringId, student_id: this.studentId }).then(data => {
      ...
      acceptRequestedOfferingBTN(offeringId, event) {
        if (!this.loadingButtonActive(event)) return
        this.acceptRequestedOffering({ emtyaz_training_id: offeringId, student_id: this.studentId }).then(data => {
          if (data.status === 'success') {
            this.successToast(data.message)
            // refresh all courses again
            this.updateTables()
      ...
      rejectRequestedOfferingBTN(offeringId, event) {
        if (!this.loadingButtonActive(event)) return
        this.rejectRequestedOffering({ emtyaz_training_id: offeringId, student_id: this.studentId }).then(data => {
          if (data.status === 'success') {
            this.successToast(data.message)
            // refresh all courses again
            this.updateTables()
