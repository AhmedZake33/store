      ...
      <template #customTable>
        ...
        >
          <template #cell(actions)="data">
            <div>
              <b-link
                :to="{ name: 'emtyaz_offerings', params: { id: data.item.id } }"
              >
                <feather-icon
                  v-b-tooltip.hover="$t('Global.show_students')"
                  icon="UsersIcon"
          ...
          <template #cell(bylaws)="data">
            <div v-if="data.item.emtyaz_type.bylaw">
              <div class="d-flex">
                {{
                  $i18n.locale == "ar"
                    ? data.item.emtyaz_type.bylaw.name_local
                    : data.item.emtyaz_type.bylaw.name
                }}
              </div>
            </div>
          </template>
          <template #cell(name)="data">
            <div v-if="data.item.emtyaz_type.bylaw">
              <div class="d-flex">
                {{
                  $i18n.locale == "ar"
                    ? data.item.emtyaz_type.name_local
                    : data.item.emtyaz_type.name
                }}
              </div>
            </div>
          </template>
          <template #cell(mandatory)="data">
            <div v-if="data.item.emtyaz_type.bylaw">
              <div class="d-flex">
                {{
                  $i18n.locale == "ar"
                    ? data.item.emtyaz_type.mandatory_local
                    : data.item.emtyaz_type.mandatory
                }}
              </div>
            </div>
          </template>
          ...
          <div class="row">
            ...
            <b-col md="6">
              <b-form-group
                ...
                invalid-feedback="End At is required"
              >
                <validation-provider
                  #default="{ errors }"
                  name="ends_at"
              ...
              >
                ...
                >
                  <v-select
                    ...
                    @search="fetchEmtiazFacutly"
                    :placeholder="$t('Global.name')"
                    @input="handleFacultySelection"
                  >
                    <template v-slot:option="option">
                      {{ option.name }}
                    </template>
              ...
              >
                ...
                >
                  <v-select
                    ...
                    class="w-100 mb-2"
                    :label="getSelectLabel()"
                    @search="fetchEmtiazBylaw"
                    :placeholder="$t('Global.name')"
                    :disabled="isEmtiazBylawDisabled()"
                    @input="handleBylawSelection"
                  >
                    <template v-slot:option="option">
                      {{ option.name }}
                    </template>
              ...
              >
                ...
                >
                  <v-select
                    ...
                    :placeholder="$t('Global.name')"
                    :disabled="isEmtiazTrainingDisabled()"
                  >
                    <template v-slot:option="option">
                      {{ option.name }}
                    </template>
            ...
            <b-col md="6">
              <b-form-group
                :label="$t('Global.academic_year')"
                label-for="emtyaz_year-input"
                invalid-feedback="Emtyaz Year is required"
              >
                <validation-provider
                  #default="{ errors }"
                  name="emtyaz_year"
...
export default {
  ...
  data() {
    return {
      mandatoryOption: [
        { id: 0, name: "No", name_local: "لا" },
        { id: 1, name: "Yes", name_local: "نعم" },
      ],
      dueDateModel: false,
      id: null,
      year: [],
      ...
      faculty: { name: null, id: null },
      bylaw: { name: null, id: null },
      filter: {
        export: 0,
        removed: 0,
        year: null,
  ...
  methods: {
    ...
    add() {
      this.id = null;
      this.faculty = { name: null, id: null };
      this.bylaw = { name: null, id: null };
    ...
    fetchEmtiazFacutly(search, load) {
      if (!this.waitFetchTraining) {
        this.waitFetchTraining = true;
        load(true);
        const query = {
          emtyaz_faculties: search,
        };

        this.getLookups(query).then((response) => {
          this.facultyOption = response.success.faculties;
          load(false);
    ...
    fetchEmtiazBylaw(search) {
      if (!this.waitFetchTraining) {
        this.waitFetchTraining = true;
        // load(true);
        const query = {
          emtyaz_bylaws: search,
        };

        this.getLookups(query).then((response) => {
          this.bylawOption = response.success.bylaws;
          // load(false);
          this.waitFetchTraining = false;
        });
      }
    ...
    init(query) {
      this.$store
        .dispatch("Emtyaz/trainings", { query: query })
        .then((res) => {
          this.EmtiazTrainingType = res.meta.lookup.training_types;
          this.facultyOption = res.meta.lookup.faculties;
          this.fetchEmtiazBylaw(this.faculty.id);
          // this.bylawOption = res.meta.lookup.bylaws;
    ...
    cancel() {
      ...
      this.emtyaz_id = null;
      this.year_training = { name: null, id: null };
      this.bylaw = { id: null, name: null, name_local: null };
      this.faculty = { id: null, name: null, name_local: null };
      // this.trainingType = data.emtyaz_type.name;
      this.capacity = null;
      this.startsAt = null;
      this.endsAt = null;
    ...
    submitDueDate() {
      ...
      this.$refs.c.validate().then((success) => {
        if (success) {
          ...
          if (this.id) {
            let payload = {
              course: Object.assign({}, course, { id: undefined }),
              id: this.id,
            };
            this.putCourse(payload)
              .then(() => {
                this.$swal({
                  icon: "success",
                  title:
    ...
    onEditClick(data) {
      this.faculty = {
        name: data.emtyaz_type.bylaw.faculty.name,
        name_local: data.emtyaz_type.bylaw.faculty.name_local,
        id: data.emtyaz_type.bylaw.faculty_id,
      };
      // this.handleFacultySelection(this.faculty.id);
      this.fetchEmtiazBylaw(this.faculty.id);
      this.bylaw = {
        name: data.emtyaz_type.bylaw.name,
        name_local: data.emtyaz_type.bylaw.name_local,
        id: data.emtyaz_type.bylaw.id,
      };
      // this.handleBylawSelection(this.bylaw.id);
      this.fetchEmtiazTraining(this.bylaw.id);
      this.emtyaz_id = data.emtyaz_type.id;
      this.id = data.id;
      this.name = { name: data.emtyaz_type.name, id: data.emtyaz_type.id };
      this.year_training = {
        name: data.year ? data.year.name : null,
        id: data.year ? data.year.id : null,
      };
    ...
    trash(id) {
      this.$swal({
        ...
        if (result.value) {
          this.$store
            .dispatch("Emtyaz/deleteTrainingType", { id: id })
            .then((_) => {
              this.init();
              this.$swal({
                icon: "success",
    ...
    handleBylawSelection() {
      if (this.faculty.id && this.bylaw.id) {
        this.name.id = null;
        this.fetchEmtiazTraining(this.bylaw.id);
        console.log("bylaw", this.bylaw.id);
      }
    },
    ...
    isEmtiazTrainingDisabled() {
      // Disable the second v-select if the first one is not selected
      return !this.bylaw.id;
    },
  },
  computed: {
    ...mapGetters({
