                    ...
                    <div class="d-flex flex-column ml-1">
                      <div class="mb-1">
                        <h2 class="mb-0" style="font-weight: bold">
                          {{
                            $i18n.locale == "ar" 
                              ? item.emtyaz_type.bylaw.faculty.name_local
                              : item.emtyaz_type.bylaw.faculty.name
                          }}
                        </h2>

                        <h4 class="card-text d-block">
                          {{
                            $i18n.locale == "ar" 
                              ? item.emtyaz_type.name_local
                              : item.emtyaz_type.name
                          }}
                        </h4>
                      </div>
                    </div>
                      ...
                      <b-row>
                        ...
                        >
                          ...
                          <div style="margin: 3px">
                            <b-button  v-if=" this.item.status.id === 0"
                              class="btn w-100"
                              variant="primary"
                              style="float: right; font-size: 12px"
                              @click="submitStatus"
                          ...
                          <!-- <div
                            v-if="
                            (status != 'Frozen'&& status != 'Published') &&
...
// eslint-disable-next-line import/extensions
// import instructors from "@/views/emtyaz/offerings/components/instructors";
// import Studies from "@/views/study/studies/components/studies";
// eslint-disable-next-line import/extensions
//import Programs from '@/views/emtyaz/offerings/components/programs'
// import Statistics from '@/views/components/info/statistics'
// import addPrograms from "@/views/emtyaz/offerings/components/addPrograms";
// import addInstructors from "@/views/emtyaz/offerings/components/addInstructors";
//import Programs from "@/views/academic_settings/programs/components/programs";
// import Programs from "@/views/emtyaz/offerings/components/programs";
// import Sections from "@/views/emtyaz/offerings/components/sections/sectionsView.vue";
import EditOffering from "@/views/emtyaz/offerings/components/edit_offering.vue";
import { EventBus } from "@/main";
import Calendar from "@/views/study/schedule/offerings/scheduling/calendar/Calendar.vue";
import ExamsSchedules from "@/views/emtyaz/offerings/exams_schedules/components/exams_schedules.vue";
import linkedOffering from "@/views/emtyaz/offerings/components/linkedOffering";
import HandOnTable from "@/views/emtyaz/offerings/components/handontable";
import {
  mdiDataMatrix,
  mdiBarcode,
...
import Statistics from "@/views/emtyaz/offerings/components/statistics.vue";
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      load: "Emtyaz/load",
      markLoad: "app/load",
      questionsArray: "Emtyaz/questions",
      specsLoad: "app/load",
    }),
    id() {
        ...
        this.offeringStudentsGrades.forEach((element) => {
          ...
          db.code =
            ...
            element.student.user.code
              ...
              db.grade = element.grade  ? element.grade : '';
          db.name =
          ...
          db.grade_type = element?.grade_type
            ? element.grade_type.gpa + "_" + element.grade_type.id
            : "";
          ...
          // if (this.hasPermission('show_total')) {
            // Math.round(
          db.markPercentage = Number.isInteger(markPercentage) ? Number(markPercentage) + "%" : Number(markPercentage).toFixed(2) + "%";
          // }
          let totalmarks = 0;
          const stat = {};
          const status = [];
          ...
          if(this.$i18n.locale === "en"){
            ...
            db.section = element.section
                ? element.section.name_local
                : ""; 
          }
          if(this.$i18n.locale === "en"){
            db.group = element.section && element.section.group && element.section.group.name
              ? element.section.group.name
              : "";
          }else{
            db.group = element.section && element.section.group && element.section.group.name_local
          ...
          db.extra = element.extra != null ? Math.round(element.extra) : "";
          // }
          // totalmarks = 0
          this.marks = true;
          studentData.push(db);
    ...
    getCourseDetails() {
      ...
      if (this.item) {
        this.course_id = this.item && this.item.course  && this.item.course.id? this.item.course.id : null;
        this.bylaw_id = this.item.course && this.item.course.bylaw && this.item.course.bylaw.id ? this.item.course.bylaw.id : null;
        //data.code = this.item.course ? this.item.course.code : '-';
        data.code = {
          value: this.item.course ? this.item.course.code : "-",
          link: this.$router.resolve({
            name: "course",
            params: { id: this.item.course && this.item.course.id ? this.item.course.id : null },
          }).href,
        };
        data.english_name = {
  ...
  methods: {
    ...mapActions({
      ...
      statusSubmit: "Emtyaz/statusSubmit",
      ...
      this.$swal({
        title:
          this.$i18n.locale == "en"
            ...
            : "هل أنت متأكد أنك تريد (تسجيل الدرجات )",
        text:
          this.$i18n.locale == "en"
            ? "Are you sure u want to submit grades?"
            : "متأكد من تسجيل الدرجات",
        icon: "warning",
