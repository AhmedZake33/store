    ...
    <b-overlay :show="loading">
      ...
      >
        ...
        >
          <template #thead-top="">
            <b-tr>
              <b-th variant="primary" class="text-center" colspan="5"> {{ $t('Global.term') }} ({{ i }}) </b-th>
            </b-tr>
          </template>
...
export default {
  ...
  methods: {
    ...mapActions({
      getStudyPlan: 'offeringsRegistrations/getStudyPlan',
    }),
    init() {
      this.loading = true
      this.getStudyPlan({ student_id: this.studentId }).then(data => {
        this.item = data
      }).catch( error => {
