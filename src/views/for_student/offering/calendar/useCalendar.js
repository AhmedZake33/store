...
export default function userCalendar() {
  ...
  const routeParamsId =  isNaN(num) ? getAuth().id : num ;
  ...
  const addEvent = eventData => {
    ...
    payload.section = eventData.section
  ...
  const fetchEvents = (info, successCallback) => {
    ...
    store
      .dispatch('calendars/fetchEvents', { calendars: {types: selectedCalendars.value , date: info ? info.startStr.split('T')[0] : null } , id : routeParamsId , type: 'student'} )
      .then(response => {
