            ...
            <b-card v-if="data">
              ...
              <div>
                ...
                >
                  ...
                  >
                    <tr
                      v-for="(field,index) in data.payment_setting.payment_accounts"
                      :key="index"
                    >
...
export default {
  ...
  data() {
    return {
      errorsdata: {},
      message: null,
      formInstallments: null,
      formInstallmentsData: [],
      formServicesRemoved: [],
      formServices: [],
      totalSumServices: {
        totalLocal: 0,
  ...
  methods: {
    ...mapActions({
      updateLevelPaymentSettings: 'yearPayments/updateLevelPaymentSettings',
    }),
    init() {
      this.formInstallments = this.data.payment_installments
      ...
      /*this.data.payment_installments.forEach((item, index) => {
        this.formInstallmentsData[index] = {
          payment_setting_installment_id: item.id,
          value_local: item.value_local,
          value_usd: item.value_usd,
        }
      })
      this.formInstallments = this.formInstallmentsData*/
    ...
    save() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.updateLevelPaymentSettings(payload).then(response => {
            this.$swal({
              icon: 'success',
    ...
    removeCard(index) {
      this.reRender = false
      const ritem = this.formServices[index] ? this.formServices[index].payment_setting_account_id : null
      const removedItem = this.formServices[index].payment_setting_account_id
        delete this.formServices[index]
      if (ritem) {
        this.formServicesRemoved.push(ritem)
      }
      this.reRender = true
    },
