      ...
      >
        <b-card v-if="data">
          ...
          <div>
            <h4>
              {{ $t('Global.ttb') }} :
              <small>{{ getSwitchAfterLabel(data.options_value.after) }}</small>
            </h4>
          </div>
          <hr>
          <div>
          ...
          <div>
            ...
            <table
              v-if="data && data.payment_accounts"
              class="table table-striped custom_table text_nobreak"
            >
              <thead>
              ...
              <tbody style="text-align: initial">
                <tr
                  v-for="field in data.payment_accounts"
                  :key="field.id"
                >
                  <td>
...
export default {
  ...
  watch: {
    data(val) {
      val.payment_accounts.forEach(item => {
