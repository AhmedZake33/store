            ...
            <b-card v-if="formPaymentSetting">
              <b-card-header class="mt-0 pt-0 pb-0 p-0">
                <div class="mt-0 d-flex align-items-center">
                  <h3>
                     {{this.title ? $t('Select faculty and program') : $t('Edit Tuition Fees')}}
                  </h3>
                </div>
                <div class="mb-2">
                  <feather-icon
                        ...
                        <td width="100%">
                          <div>
                            <b-form-group>
                              ...
                              >
                                <v-select
                                    v-model="formPaymentSetting.faculty_id"
                    ...
                    <td>
                      ...
                      >
                        ...
                        >
                          <b-form-input
                              type="number"
                              v-model="trainingFees.value_local"
                    ...
                    <td>
                      ...
                      >
                        ...
                        >
                          <b-form-input
                              type="number"
                              v-model="trainingFees.value_usd"
                              :disabled="true"
                              :state="errors.length > 0 ? false : null"
                          />
                          <ValidationErrors
...
export default {
  ...
  watch: {
    'formPaymentSetting.faculty_id'(value) {
      this.refreshLookup(value)
      this.formPaymentSetting.program_id = null
    },
  ...
  methods: {
    ...
    init() {
      ...
      if (this.data) {
        ...
        this.trainingId = this.trainingFees.id
      }
    },
    refreshLookup(faculty_id = null) {
      this.getLookups({ faculties: true, faculty_id_programs: faculty_id, pre_certificates: true }).then(_ => {});
    },
    addNewCard() {
    ...
    changeServiceValueNew(){
      this.trainingFees= {
        id: this.trainingId != null ? this.trainingId : null,
        value_local: 0,
        value_usd: 0,
      },
      ...
      this.formServices.forEach((item, index) => {
        if (item.value_local) {
          ...
          this.trainingFees.value_local += parseInt(item.value_local)
        }
        if (item.value_usd) {
          this.totalSumServices.totalUsd += parseInt(item.value_usd)
          this.trainingFees.value_usd += parseInt(item.value_usd)
        }
      })
    },
    save() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.formInstallments.push(this.trainingFees)
          const payload = {
            yearId: this.yearId,
