...
export default {
  ...
  methods: {
    ...
    changeServiceValueNew(){
      this.examFees= {
        id: this.examId != null ? this.examId : null,
        value_local: 0,
        value_usd: 0,
      },
      ...
      this.formServices.forEach((item, index) => {
        if (item.value_local) {
          ...
          this.examFees.value_local += parseInt(item.value_local)
        }
        if (item.value_usd) {
          this.totalSumServices.totalUsd += parseInt(item.value_usd)
          this.examFees.value_usd += parseInt(item.value_usd)
        }
      })
    },
    save() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          this.formInstallments.push(this.examFees)
          const payload = {
            yearId: this.yearId,
