  ...
  <b-row class="new_edit" :class="load_data ? 'disabled_all' : ''">
    <b-col cols="12">
      <validation-observer ref="simpleForm">
        <b-form v-if="userData" ref="form" @submit.stop.prevent="save">
          <ShowComponent v-show="tabNumber == 0" :data="userData" :loading="load" :only="['code', 'name', 'name_local', 'id', 'type', 'gender', 'birth_date', 'access', 'insurance_no']">
            <template #edit>
              <div class="mb-2">
            ...
            <template #type="{ data }" v-if="$route.name == 'user-add' && !id">
              <th scope="row" :width="'20%'">
                {{ $t(`Global.type`) }}
              </th>
              <td>
            ...
            <template #code>
              ...
              <td>
                <b-form-group v-if="codeKey" label-for="vi-postal-code">
                  <validation-provider #default="{ errors }" name="Code" :rules="$route.params.id ? 'code' : 'code|required'">
                    <b-form-input :disabled="canEditCode" :placeholder="$t('Global.code')" :state="errors.length > 0 ? false : null" id="vi-postal-code" v-model="form.code" />
          ...
          <ShowComponent
            v-show="type == 1 && tabNumber == 1 && $route.name != 'user-show'"
            :data="userData"
            :loading="load"
            :only="['id', 'rank', 'department', 'employee_type', 'teacher_at', 'teaching_assistant_at', 'professor_at', 'seniority', 'msc_from', 'msc_from_local', 'phd_from', 'phd_from_local', 'phd_at', 'assistant_professor_at', 'emeritus_at']"
          >
            <template #edit>
              <div class="mb-2">
                <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_users')" />
            ...
            <template #rank="{ data }">
              ...
              <td>
                <b-form-group>
                  <validation-provider #default="{ errors }" rules="required" name="rank" v-if="type == 1">
                    <v-select
            ...
            <template #id="{ data }">
              ...
              <td>
                <b-form-group>
                  <validation-provider #default="{ errors }" rules="required" name="faculty_id" v-if="type == 1">
                    <v-select
            ...
            <template #department="{ data }">
              ...
              <td>
                <b-form-group>
                  <validation-provider #default="{ errors }" rules="required" name="department_id" v-if="type == 1">
                    <v-select
            ...
            <template #employee_type="{ data }">
              ...
              <td>
                <b-form-group>
                  <validation-provider #default="{ errors }" rules="required" name="degree" v-if="type == 1">
                    <v-select
          ...
          <ShowComponent v-show="(type == 1 && tabNumber == 2 && $route.name != 'user-show') || (tabNumber == 1 && type != 1) || (type == 1 && $route.name == 'user-show' && tabNumber == 1)" :data="userData" :loading="load" :only="['email', 'personal_email', 'mobile', 'phone', 'internal_phone']">
            <template #edit>
              <div class="mb-2">
                <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_users')" />
              </div>
              ...
              <td>
                <b-form-group label-for="vi-email">
                  <validation-provider #default="{ errors }" :rules="$route.params.id ? 'email' : 'email|required'" name="email">
                    <b-form-input :disabled="canEditAcademicEmail" @keypress="sanitizeEnglish($event);" :placeholder="$t('academic_email')" :state="errors.length > 0 ? false : null" id="vi-email" type="email" v-model="form.email" placeholder="john.doe@email.com" />
                    <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'email')" />
                  </validation-provider>
                </b-form-group>
              </td>
            </template>
              ...
              <td>
                <b-form-group label-for="vi-personal-email">
                  <validation-provider #default="{ errors }" :rules="$route.params.id ? 'email' : 'email'" name="email">
                    <b-form-input @keypress="sanitizeEnglish($event);" :state="errors.length > 0 ? false : null" id="vi-personal-email" type="email" v-model="form.personal_email" :placeholder="$t('Global.personal_email')" />
                    <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'personal_email')" />
                  </validation-provider>
                </b-form-group>
              </td>
            </template>
          ...
          <ShowComponent v-show="(type == 1 && tabNumber == 3) || (tabNumber == 2 && type != 1) || (type == 1 && $route.name == 'user-show' && tabNumber == 2)" :data="userData" :loading="load" :only="['address', 'postal_code']">
            <template #edit>
              <div class="mb-2">
                <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_users')" />
              </div>
...
import { BRow, BCol, BOverlay, BCard, BForm, BFormGroup, BFormInput, BFormDatepicker, BFormTextarea } from 'bootstrap-vue';
  ...
  methods: {
    ...
    init() {
      ...
      if (this.form.faculty_id) {
        this.query.faculty_id = this.form.faculty_id;
      }
      if (this.id) {
    ...
    save() {
      if (!this.userData.code && this.tabNumber != 0) {
        this.form.code = '91291271990';
      }
      if (!this.userData.gender && this.tabNumber != 0) {
        this.form.gender = 1;
      }
      if (!this.userData.gender && this.tabNumber != 0) {
        this.form.gender = 1;
      }
      if ((!this.userData.national_id || this.userData.national_id.length != 14) && this.tabNumber != 0) {
        this.form.national_id = 12345678912345;
      }
      this.$refs.simpleForm.validate().then(success => {
        if (success || ((this.$route.params.id && this.department_id) || (this.$route.params.id && this.type == 2))) {
          if (this.id) {
            this.form.type = this.type || 3;
          ...
          this.$store
            ...
            .then(response => {
              ...
              if (this.$route.params.id) {
                return this.$emit('hideEdit');
                if (this.type == 1) {
                  // this.$router.push({ name: 'staff-show', params: { id: this.$route.params.id } });
                  return this.$emit('hideEdit');
                } else if (this.type == 2) {
                  // this.$router.push({ name: 'employee-show', params: { id: this.$route.params.id } });
                  return this.$emit('hideEdit');
                } else {
                ...
                if (this.type == 1) {
                  ...
                  this.$router.push({ name: 'employee-show', params: { id: response.data.id } });
                } else {
                  this.$router.push({ name: 'user-show', params: { id: response.data.id } });
                }
              }
            })
            .catch(error => {
              this.errorsdata = this.handleBackendError(error.response.data.errors);
              // if (this.type == 1 && this.$route.params.id) {
              //   // if (this.errorsdata.code) {
              //   //   // this.$refs.wizard.changeTab(0, 0);
              //   // }
              //   // if (this.errorsdata.email) {
              //   //   // this.$refs.wizard.changeTab(2, 2);
              //   // }
              // }
            });
        } else {
          ...
          if (!this.tabNum && this.$route.name == 'staff-add' && this.form.code && this.form.name && this.form.name_local && this.form.national_id && this.form.gender && this.form.birth_date) {
            document.getElementById('academic_info').click();
          }
          if (this.tabNumber == 1 && this.$route.name == 'staff-add' && this.form.faculty_id && this.form.department_id && this.form.rank_id && this.form.employment_id) {
