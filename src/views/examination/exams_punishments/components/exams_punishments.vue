<template>
  <div>
    ...
    >
          ...
          <template #cell(actions)="data">
            <div v-if="data">
              ...
              <feather-icon
                ...
                v-if="hasPermission('delete_examPunishment')"
                icon="Trash2Icon"
                style="cursor: pointer"
                class="text-danger"
                @click="deletePunishment(data.item.barcode)"
              />
            </div>
          </template>
        </b-table>
      ...
      <template #action>
        <b-button
          v-if="hasPermission('edit_examPunishment')"
          @click="openModal()"
          class="btn-icon"
...
export default {
  ...
  computed: {
    ...mapGetters({
      items: "examsPunishments/items",
      total: "examsPunishments/total",
      needDrop:'app/needDrop',
      load: "examsPunishments/load",
      customLoad: "examsPunishments/customLoad",
      lookups: "examsPunishments/lookups",
      student_info: "examsPunishments/student_info",
    }),
    exam_id_param() {
  ...
  watch: {
    ...
    "form.term"(val) {
      this.form.registeration_id = null;
      this.$store
        .dispatch("examsPunishments/get_student_info", {
  ...
  methods: {
    ...
    refresh(query) {
      ...
      this.$store
        .dispatch("examsPunishments/examsPunishments", { query: query })
        .then((data) => {
    ...
    openModal() {
      this.modalTitle = this.$t("Global.add_exam_punishment");
      ...
      if (this.items) {
        this.$swal({
          ...
          if (result.value) {
            ...
            this.$store
              .dispatch("examsPunishments/remove", id)
              .then((response) => {
                this.$swal({
                  icon: "success",
    ...
    submit(save_and_close) {
      this.$refs.simpleRules.validate().then((success) => {
        if (success) {
          ...
          this.$store
            .dispatch("examsPunishments/put", payload)
            .then((response) => {
              this.$swal({
                icon: "success",
