<template>
  <div>
    ...
    >
      <template #action>
        ...
        <b-button
          ...
          v-if="hasPermission('export_department')"
          class="btn-icon ml-1"
      ...
      <template v-if="lookups && !inside" #filter>
        <b-col cols="6" md="4">
          <v-select
            :filter="fuseSearch"
    ...
    <Modal :hint="$t('Global.importMigrationData')" :link="'/departments/import'" :title="$t('Global.importMigrationData')" :show="importMigrationModal"></Modal>
  </div>
</template>
