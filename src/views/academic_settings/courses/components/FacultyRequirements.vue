...
export default {
  ...
  computed: {
    ...mapGetters({
      items: "courses/facultyRequirements",
      total: "courses/totalFacultyRequirements",
  ...
  methods: {
    ...
    refresh(query) {
      ...
      this.$store.dispatch("courses/facultyRequirements", { id: this.program_id, query: query }).then(_=>{
      });
    },
    reset() {
      this.filter = {}
    ...
    Export(query) {
      ...
      this.$store.dispatch("courses/exportFacultyRequirements", query).then((_) => {
