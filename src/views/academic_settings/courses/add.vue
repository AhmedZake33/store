<template>
  <!-- v-if="!id || (id && courseData.offered_now == 0)" -->
  <div class="new_edit">
    <!-- <Add v-if="$route.name == 'course_add'"></Add> -->
        ...
        <b-card class="mb-0" :class="load_data ? 'disabled_all' : ''">
          <b-tabs v-model="tabIndex" @input="saveStatus" vertical nav-wrapper-class="nav-hidden" align="left">
            <b-tab>
              ...
              >
                ...
                <template #name="{ data }">
                  ...
                  <td>
                    <b-form-group label-for="name-input" invalid-feedback="English Name is required">
                      <validation-provider #default="{ errors }" name="name" rules="english|required">
                        <b-form-input :placeholder="$t('Global.english_name')" @keypress="sanitizeEnglish($event)" id="name-input" v-model="form.name" :state="errors.length > 0 ? false : null" @blur="checkIfArabic('en')" />
                        <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'name')" />
                      </validation-provider>
                ...
                <template #name_local="{ data }">
                  ...
                  <td>
                    <b-form-group label-for="nameAr-input" invalid-feedback="Arabic Name is required">
                      <validation-provider #default="{ errors }" name="name_local" rules="arabic|required">
                        <b-form-input :placeholder="$t('Global.arabic_name')" @keypress="sanitizeArabic($event)" id="nameAr-input" v-model="form.name_local" :state="errors.length > 0 ? false : null" @blur="checkIfArabic('ar')" />
                        <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'name_local')" />
                      </validation-provider>
                ...
                <template #continuous="{ data }" v-if="showContinousAndfinal">
                  ...
                  <td>
                    <b-form-checkbox v-model="form.continuous" style="margin-top: 6px" class="mt-0 custom-control-primary"/>
                  </td>
                </template>
                <template #graduation_flag="{ data }">
                  ...
                  <td>
                    <b-form-checkbox v-model="form.graduation_flag" style="margin-top: 6px" class="mt-0 custom-control-primary"/>
                  </td>
                </template>
              </ShowComponent>
            ...
            <b-tab>
              ...
              <ShowComponent :data="id ? courseData : { id: '', description: '', description_local: '',course_aims:'',reference:'' }" :only="['id', 'description', 'description_local','course_aims','reference']" :loading="load">
                <template #edit>
                  <div class="mb-2">
                    <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_course')" />
                  </div>
                ...
                <template #course_aims="{ data }">
                  ...
                  <td>
                    <b-form-group label-for=" description-input" invalid-feedback="lecture hours is required">
                      <validation-provider #default="{ errors }" rules="address" name="course_aims">
                        <b-form-textarea :placeholder="$t('Global.course_aims')" @keypress="sanitizeEnglish($event)" id=" description-input" v-model="form.course_aims" rows="3" />
                        <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'course_aims')" />
                      </validation-provider>
                    </b-form-group>
                  </td>
                </template>
                 <template #reference="{ data }">
                  ...
                  <td>
                    <b-form-group label-for=" description-input" invalid-feedback="lecture hours is required">
                      <validation-provider #default="{ errors }" rules="address" name="reference">
                        <b-form-textarea :placeholder="$t('Global.reference')" @keypress="sanitizeEnglish($event)" id=" description-input" v-model="form.reference" rows="3" />
                        <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'reference')" />
                      </validation-provider>
                    </b-form-group>
                  </td>
                </template>
                  ...
                  <td>
                    <b-form-group label-for=" description-input" invalid-feedback="lecture hours is required">
                      <validation-provider #default="{ errors }" name="description">
                        <b-form-textarea :placeholder="$t('Global.english_description')" id=" description-input" v-model="form.description" rows="3" />
                ...
                <template #description="{ data }">
                  ...
                  <td>
                    <b-form-group label-for=" description_local-input" invalid-feedback="lecture hours is required">
                      <validation-provider #default="{ errors }" name="description">
                        <b-form-textarea :placeholder="$t('Global.arabic_description')" id=" description_local-input" v-model="form.description_local" rows="3" />
                        <ValidationErrors :frontend-errors="errors" :backend-errors="getBackendFieldError(errorsdata, 'description')" />
                      </validation-provider>
                    </b-form-group>
                  </td>
            ...
            <b-tab>
              ...
              >
              <ShowComponent :data="id ? courseData : { credit_hours: '', equivalent_ects: '', required_swl: '' }" :only="['credit_hours', 'equivalent_ects', 'required_swl']" :loading="load">
                <template #edit>
                  <div class="mb-2">
                    <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_course')" />
                  </div>
            ...
            <b-tab>
              ...
              >
              <ShowComponent :data="id ? courseData : { marks: '', pass_fail: '' }" :only="['marks', 'pass_fail','final_exam_first_semester']" :loading="load">
                <template #edit>
                  <div class="mb-2">
                    <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_course')" />
                  </div>
                ...
                <template #final_exam_first_semester="{ data }" v-if="showContinousAndfinal">
                  ...
                  <td>
                    <b-form-checkbox v-model="form.final_exam_first_semester" style="margin-top: 6px" class="mt-0 custom-control-primary" />
                  </td>
                </template>
                <template #marks="{ data }">
                  <th scope="row" :width="'40%'">
            ...
            <b-tab>
              ...
              >
              <ShowComponent :data="id ? courseData : { midterm_hours: '', finalexam_hours: '' }" :only="['midterm_hours', 'finalexam_hours']" :loading="load">
                <template #edit>
                  <div class="mb-2">
                    <feather-icon v-b-tooltip.hover="$t('Global.close')" style="cursor: pointer" icon="XCircleIcon" class="font-medium-3" @click="cancel()" v-if="hasPermission('edit_course')" />
                  </div>
...
import { BTab, BTabs, BLink, BCard, BAvatar, BTable, BTh, BTr, BButton, BCol, BForm, BBadge, BFormCheckbox, BFormGroup, BFormInput, BRow, BFormTextarea } from 'bootstrap-vue';
...
export default {
  ...
  computed: {
    ...
    showContinousAndfinal(){
      if(this.form.bylaw_id){
        let bylaw = this.options.bylaws.filter(x => x.id == this.form.bylaw_id);
        return (bylaw[0].type_id == 2)?true:false;
      }
      if(this.courseData != {}){
        return (this.courseData.bylaw && this.courseData.bylaw.type.id == 2)?true:false;
      }
      return false;
    ...
    departments() {
      ...
      if (this.id && this.item.department && this.item.faculty) {
        ...
        if (pos == -1 && this.item.department.removed == 0) {
    ...
    query() {
      const data = {
        bylaws: this.form?.bylaw_id ? this.form?.bylaw_id : true,
        // departments: true,
        faculties: this.form?.faculty_id ? this.form?.faculty_id : true,
        faculty_id: this.form.faculty_id ? this.form.faculty_id : this.$route.params.faculty_id,
        bylaw_id: this.form.bylaw_id || this.$route.params.bylaw_id,
        courses: this.item?.course_id ? this.item.course_id : true,
    ...
    core_requisites_courses() {
      if (this.options.courses) {
        return this.options.courses.filter(el => el.id != this.form.exposed_requisites && el.id != this.id);
      }
    },
    exposed_requisites_courses() {
      if (this.options.courses) {
        return this.options.courses.filter(el => el.id != this.form.co_requisites && el.id != this.id);
      }
    },
    id() {
  ...
  methods: {
    ...
    updateCat(data, prop, index) {
      this.form.marks.forEach(element => {
        if (prop == element.id) {
          ...
          this.$toast(
            {
              ...
              props: {
                icon: 'InfoIcon',
                text: this.$t('Global.assessment_once_error'),
                variant: 'danger',
              },
            },
            {
    ...
    save() {
      this.$refs.simpleForm.validate().then(success => {
        if (success) {
          //|| (this.$route.params.id && this.department_id)
          let payload = {};
          payload = {
            id: this.id || null,
            query: this.form,
          ...
          let empty = this.form.marks.filter(x => x.id == null);
          if(this.form.marks){
            if(empty.length != 0 && this.tabIndex == 0 && this.form.name && this.form.name_local && this.form.faculty_id && this.form.department_id && this.form.bylaw_id){
              this.tabIndex = 3
            }
          }
          this.$swal({
          ...
          if (this.tabIndex == 3 && empty.length == 0) {
            this.tabIndex = 0;
          }
          // if (!success) {
          //     if(!this.department_id && this.tabIndex != 0){
          //         this.tabIndex = 0;
          //     }
          //   this.$toast(
          //         {
    ...
    cancel() {
      ...
      if (this.inline) {
        this.updateFilterQueryParams({ inline: false });
    ...
    init(id) {
      if (this.id) {
        this.getItem({ id: this.id, type: 4 }).then(response => {
          let payload = this.item ? { ...this.query, bylaw_id: this.item.bylaw_id } : this.query;
          this.lookups(payload).then(_ => {
            if (this.id) {
              this.form.description = this.item?.description;
              this.form.description_local = this.item?.description_local;
              this.form = this.item;
              this.form.faculty_id = this.item?.faculty?.id;
              if (this.item.department.removed == 0) {
